 ...
 <div class=" col ">
     <div class="container">
         <h3 class="text-center my-5">DANH SÁCH KHÁCH HÀNG</h3>
         <div class="d-flex ">
             <!-- thêm sản phẩm -->

             <a href="index.php?url=customer&act=add"
                 class="bg-success h-25 p-1 px-2 rounded-2 text-light m-3 text-decoration-none">Thêm khách
             ...
             <table class="table table-hover table-bordered text-center">
                 ...
                 <tbody>
                     ...
                     <tr>
                         ...
                         <td><?=$item['name']?></td>
                         <td><?=$item['phone']?></td>
                         <td><?php $flag = 0; for($i = 0; $i < strlen($item['email']); $i++) {
                            echo $item['email'][$i];
                            if($item['email'][$i] === '@' || $item['email'][$i] ==='.') {
                                echo " </br> ";
                            }
                        }?></td>
                         <td><?php $flag = 0; for($i = 0; $i < strlen($item['address']); $i++) {
                            echo $item['address'][$i];
             ...
             <nav aria-label="Page navigation example">
                 <ul class="pagination">
                     <li class="page-item"><a class="page-link border-danger"
                             href="index.php?url=customer&act=list&filter=<?php echo isset($fil)?$fil:0?>">1</a>
                     </li>
                     <?php
                                ...
                                ?>
                     <li class="page-item "><a class="page-link border-danger"
                             href="index.php?url=customer&act=list&filter=<?php echo isset($fil)?$fil:0?>&pagenum=<?=$page?>">
                             <?=$page?>
                         </a>
                     </li>
                     <?php }} ?>
