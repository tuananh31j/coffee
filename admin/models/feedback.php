...
function getListFB($kw, $fil, $offset)
{
    ...
    $sql = "select feedback.*, customer.name as nameCus, product.name as namePro, product.status from feedback ";
    ...
    $sql .= "where $keyword and product.status = 1 group by feedback_id $filter limit 10  offset $offset";
...
function getAllFB($kw, $fil)
{
    ...
    $sql .= "inner join customer on customer.customer_id = feedback.customer_id ";
    $sql .= "inner join product on product.product_id = feedback.product_id ";
    $sql .= "where $keyword and product.status = 1 group by feedback_id $filter";
    return pdo_query($sql);
}
// xoa FB
